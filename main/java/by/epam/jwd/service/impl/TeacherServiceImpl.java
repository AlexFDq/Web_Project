...
public class TeacherServiceImpl extends UserServiceImpl implements TeacherService {
    ...
    @Override
    public Test changeTest(int testId, String newName, String newTime, int newCategoryId) throws ServiceException {
        checkValues(testId, newName, newTime, newCategoryId);

        Test newTest;

        try {
            newTest = getTeacherDAO().changeTest(testId, newName, newTime, newCategoryId);
        } catch (DAOException e) {
    ...
    @Override
    public Group formGroup(int groupId, String name) throws ServiceException {
        if (groupId <= 0) {
        ...
        try {
            group = getTeacherDAO().formGroup(groupId, name);
        } catch (DAOException e) {
